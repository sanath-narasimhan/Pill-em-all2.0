...
class backend():
    ...
    def topk(self, query, k=20): #which=0,
        ...
        #df = pd.DataFrame( columns=['ReviewID','Drug Name','Useful count','Condition','Rating(/10)','Review','Similarity'])
        for row in out:
           # print(row)
            review = row[-2]
            
            row[-2] = " ".join(["<b><i><mark>"+word+"</mark></i></b>" if word.replace(".","").replace("!","").replace("?","").replace("\\","") in q_vect else word for word in review.split()])
            #for w in q_vect: review.replace(w, "<i><b>"+w+"</b></i>")
            #row[-2] = review
            #df.append()
            #print(row[1]["Review"])
